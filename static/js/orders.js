window.onload = function(){
    ...
    function order_summary_update(orderitem_price, delta_quantity){
            delta_cost = price_arr[orderitem_price] * delta_quantity;
            order_total_cost = Number((order_total_cost + delta_cost).toFixed(fractionDigits: 2));
            orderitem_quantity = orderitem_quantity + delta_quantity;

            $('.order_total_quantity').html(order_total_quantity.toString());
            $('.order_total_cost').html(order_total_cost.toString());
        };
        $('.formset_row').formset(opts:{
             ...
             $.ajax({
                ...
                success: function(data){
                    if(data.price){
                        ...
                        var price_string = '<span>' + data.price.toString().replace(searchValue: '.', replaceValue: ',') +' </span>';
